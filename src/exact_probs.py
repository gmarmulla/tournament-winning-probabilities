...
def advanceProbsMix(P: np.ndarray, M: np.ndarray, round_el: int) -> np.ndarray:
    ...
    for b in range(0, int(nslots / 2)):  # subtree number
        for h in range(0, nteamspossperslot - 1):
            t = nteamspossperslot * b + h
            gt = int(t / groupsize)

            for k in range(h + 1, nteamspossperslot):
                s = nteamspossperslot * b + k  # s is never equal to t
                gs = int(s / groupsize)
...
def runExactProbs(M: np.array) -> np.ndarray:
